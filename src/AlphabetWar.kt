...
fun main(){
    println(alphabetWar("z"))        // Right side wins!
    println(alphabetWar("zdqmwpbs")) // Let's fight again!
    println(alphabetWar("zzzzs"))    // Right side wins!
    println(alphabetWar("wwwwwwz"))  // Left side wins!

}

fun alphabetWar(fight: String): String {
    var leftSide: Map<Char, Int> = mapOf( 'w' to 4, 'p' to 3, 'b' to 2, 's' to 1)
    var rightSide: Map<Char, Int> = mapOf('m' to 4, 'q' to 3, 'd' to 2, 'z' to 1)
    var leftPower = 0
    var rightPower = 0

    for (char in fight) {
        if (char in leftSide) {
            leftPower += leftSide[char] ?: 0
        } else {
            rightPower += rightSide[char] ?: 0
        }
